...
const InstallmentEnglishReceiptButton: React.FC<InstallmentEnglishReceiptButtonProps> = ({ receiptData, compact = false }) => {
    ...
    try {
      ...
      if ((!remainingTuitionAmount && !remainingTransportationAmount) || 
        ...
        try {
          ...
          if (user?.schoolId && receiptData.studentId) {
            const studentFeesResponse = await hybridApi.getFees(user.schoolId, receiptData.studentId);
  ...
  const generateReceiptHTML = (receiptData: any): string => {
    ...
    const englishName = receiptData.englishName || transliterateArabic(receiptData.studentName || '');
