  ...
  const saveSubscription = async (subscription: Subscription) => {
    try {
      if (selectedSubscription) {
        // Update existing subscription - update the school
        const schoolUpdateData = {
        ...
        if (updateResponse.success) {
          ...
          setSubscriptions(updatedSubscriptions);
          alert(`تم تحديث اشتراك ${subscription.schoolName} بنجاح`);
        } else {
          console.error('Failed to update subscription:', updateResponse.error);
          alert('حدث خطأ أثناء تحديث الاشتراك');
        }
      } else {
        // Add new subscription - create a new school
        const schoolData = {
          name: subscription.schoolName,
          subscription_start: subscription.subscriptionStart,
          subscription_end: subscription.subscriptionEnd,
        ...
        const saveResponse = await hybridApi.saveSchool(schoolData);
        ...
        if (saveResponse.success) {
          ...
          const newSubscription = {
            ...subscription,
            id: newId,
            schoolId: saveResponse.data?.id || newId,
            createdAt: new Date().toISOString()
          };
          
          const updatedSubscriptions = [...subscriptions, newSubscription];
          setSubscriptions(updatedSubscriptions);
          alert(`تم إضافة اشتراك ${subscription.schoolName} بنجاح`);
        } else {
          console.error('Failed to add subscription:', saveResponse.error);
