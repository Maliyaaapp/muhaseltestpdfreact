...
const defaultSettings: SchoolSettings = {
  ...
  useEffect(() => {
    const updatePreview = async () => {
      if (user?.schoolId) {
        try {
          const feePreview = await getNextReceiptNumber(user.schoolId, 'fee');
          setNextReceiptPreview(feePreview);
        } catch (error) {
          console.error('Error getting fee receipt preview:', error);
          // Fallback to direct generation
          try {
            const fallbackPreview = generateReceiptNumber(settings, 'PREVIEW-STUDENT');
            setNextReceiptPreview(fallbackPreview);
          } catch {
    ...
    updatePreview();
  }, [settings.receiptNumberFormat, settings.receiptNumberPrefix, settings.receiptNumberCounter, settings.receiptNumberYear, user?.schoolId]);

  useEffect(() => {
    const updateInstallmentPreview = async () => {
      if (user?.schoolId) {
        try {
          const installmentPreview = await getNextReceiptNumber(user.schoolId, 'installment');
          setNextInstallmentReceiptPreview(installmentPreview);
        } catch (error) {
          console.error('Error getting installment receipt preview:', error);
          // Fallback to direct generation
          try {
            const fallbackPreview = generateReceiptNumber(settings, 'PREVIEW-STUDENT', undefined, 'installment');
            setNextInstallmentReceiptPreview(fallbackPreview);
          } catch {
  ...
  }, [settings.installmentReceiptNumberFormat, settings.installmentReceiptNumberPrefix, settings.installmentReceiptNumberCounter, settings.installmentReceiptNumberYear, user?.schoolId]);
