...
const EnglishReceiptButton: React.FC<EnglishReceiptButtonProps> = ({ receiptData, fee, generateReceiptData, compact = false }) => {
  ...
  const generateReceiptHTML = (receiptData: any): string => {
    ...
    // CRITICAL FIX: Detect combined receipt more broadly
    // A combined receipt is when:
    // 1. feeType explicitly says combined/transportation_and_tuition
    // 2. OR when we have a totalAmount that's greater than individual tuition+transportation (suggesting combined)
    // 3. OR when the paid amount is greater than the sum of individual fees (suggesting combined payment)
    const explicitCombined = (receiptData.feeType === 'transportation_and_tuition') || 
                             (feeType === 'Transportation & Tuition Fees') ||
                             (receiptData.feeType === 'combined') ||
                             (typeof feeType === 'string' && feeType.toLowerCase().includes('combined'));
    
    // Get the actual total from the fee record - this is the TRUE total
    const actualTotalFromRecord = Math.max(0, Number((receiptData as any).totalAmount ?? (receiptData as any).originalAmount ?? 0));
    
    // Get the actual paid amount from the fee record
    const actualPaidFromRecord = Math.max(0, Number((receiptData as any).paidAmount ?? (receiptData as any).amount ?? 0));
    
    // Check if this looks like a combined fee based on amounts
    // If the total or paid amount is greater than the sum of individual fees, it's combined
    const sumOfIndividualFees = tuitionAfterDiscount + transportationAmount;
    const looksLikeCombined = (actualTotalFromRecord > sumOfIndividualFees && sumOfIndividualFees > 0) ||
                              (actualPaidFromRecord > sumOfIndividualFees && sumOfIndividualFees > 0) ||
                              (actualTotalFromRecord > 0 && sumOfIndividualFees === 0);
    
    // Final combined receipt detection
    const isCombinedReceipt = explicitCombined || looksLikeCombined;
    
    // For combined receipts, use the actual total from record, otherwise use calculated sum
    const combinedTotalAmount = isCombinedReceipt && actualTotalFromRecord > 0 
      ? actualTotalFromRecord 
      : Math.max(actualTotalFromRecord, totalFeesAmount);
    
    // CRITICAL FIX: Get the actual paid amount
    const explicitPaid = actualPaidFromRecord > 0 ? actualPaidFromRecord : paidAmountValue;
    
    // CRITICAL FIX: Calculate remaining amount correctly
    // First check for explicit remainingAmount or balance from the fee record
    let explicitRemaining = 0;
    if ((receiptData as any).remainingAmount !== undefined && (receiptData as any).remainingAmount !== null) {
      explicitRemaining = Math.max(0, Number((receiptData as any).remainingAmount));
    } else if ((receiptData as any).balance !== undefined && (receiptData as any).balance !== null) {
      explicitRemaining = Math.max(0, Number((receiptData as any).balance));
    } else {
      // Calculate from total - paid if no explicit remaining amount
      explicitRemaining = Math.max(0, combinedTotalAmount - explicitPaid);
    }
    
    console.log('ENGLISH RECEIPT - Combined fee calculation:', {
      isCombinedReceipt,
      explicitCombined,
      looksLikeCombined,
      actualTotalFromRecord,
      actualPaidFromRecord,
      sumOfIndividualFees,
      combinedTotalAmount,
      explicitPaid,
      explicitRemaining,
      remainingAmount: (receiptData as any).remainingAmount,
      balance: (receiptData as any).balance,
      status: receiptData.status,
      isPartialPayment
    });
    
    const combinedFullyPaid = (receiptData.status === 'paid') || (explicitRemaining === 0 && explicitPaid >= combinedTotalAmount);
    const combinedPaidAmount = explicitPaid;
    const combinedRemainingAmount = combinedFullyPaid ? 0 : explicitRemaining;
      ...
      <div class="receipt-container">
        ...
        <div class="receipt-body">
          ...
          <tbody>
              ...
              <tr style="background-color: #d4edda; color: #155724; font-weight: bold;">
                <td><strong>Amount Paid</strong></td>
                <td class="fee-amount"><strong>${transportationPaidAmount.toFixed(2)} ${currencySymbol}</strong></td>
              </tr>
              
              <tr style="background-color: #fff5f5; color: #C53030; font-weight: bold;">
                <td><strong>Remaining Amount</strong></td>
                <td class="fee-amount"><strong>${transportationRemainingAmount.toFixed(2)} ${currencySymbol}</strong></td>
              </tr>
              `
              : `
