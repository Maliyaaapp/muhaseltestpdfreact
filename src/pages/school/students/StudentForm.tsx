...
const StudentForm = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (isEditMode && id) {
        ...
        // Leave studentId empty for manual entry
        setFormData(prev => ({
          ...prev,
          studentId: ''
        }));
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'phone' || name === 'whatsapp') {
      ...
      // Just update the grade, don't auto-generate student ID
      setFormData({
        ...formData,
        grade: value
      });
      ...
      if (showTuitionFeeSection && formData.tuitionFee && !showCombinedFees) {
        ...
        if (createInstallments && installmentCount > 0 && savedFee) {
          try {
            ...
            const discountPerInstallment = tuitionDiscount / installmentCount;
      ...
      if (showTuitionFeeSection && formData.tuitionFee && showCombinedFees) {
        ...
        if (createInstallments && installmentCount > 0 && savedFee) {
          try {
            ...
            const installmentAmount = netAmount / installmentCount;
            const discountPerInstallment = combinedDiscount / installmentCount;
            const baseDate = new Date();
            for (let i = 0; i < installmentCount; i++) {
              const installmentDate = new Date(baseDate);
              installmentDate.setMonth(baseDate.getMonth() + i);
