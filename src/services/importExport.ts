...
async function supabaseBatchImport(
  ...
  const studentById = new Map<string, any>(studentsWithIds.map((s: any) => [s.id, s]));
  for (const f of feesWithIds as any[]) {
    if (!f.studentName || typeof f.studentName !== 'string' || f.studentName.trim() === '') {
      const s = studentById.get(f.studentId);
      f.studentName = (s?.name ?? '').toString();
    }
    if (!f.grade || typeof f.grade !== 'string' || f.grade.trim() === '') {
      const s = studentById.get(f.studentId);
      f.grade = (s?.grade ?? '').toString();
    }
  }
  for (const inst of installments) {
    ...
    if (!feeIdMap.has(feeKey)) {
      const s = studentById.get(resolvedStudentId) || {};
      const syntheticFee: any = {
        id: uuidv4(),
        schoolId,
        studentId: resolvedStudentId,
        studentName: s.name || '',
        grade: s.grade || '',
        feeType: (inst as any).feeType,
        description: `${(inst as any).feeType} - ${s.name || ''}`,
        amount: Math.round(Number((inst as any).amount || 0) * Number((inst as any).totalInstallments || 1)),
        discount: 0,
        paid: 0,
        balance: Math.round(Number((inst as any).amount || 0) * Number((inst as any).totalInstallments || 1)),
        status: 'unpaid',
        dueDate: (inst as any).dueDate || new Date().toISOString().split('T')[0],
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString()
      };
      feesWithIds.push(syntheticFee);
      feeIdMap.set(feeKey, syntheticFee.id);
    }
  }

  let reservedInstallmentReceiptNumbers: string[] = [];
  const paidInstallmentsCount = installments.filter((inst) => (inst as any).isPaid || (inst as any).paidDate).length;
  if (paidInstallmentsCount > 0) {
    try {
      reservedInstallmentReceiptNumbers = await reserveReceiptNumbers(schoolId, 'installment', paidInstallmentsCount);
    } catch {}
  }
  let receiptIdx = 0;
  ...
  const installmentsWithIds = installments.map((inst) => {
    ...
    const receiptNumber = ((inst as any).isPaid || (inst as any).paidDate) && reservedInstallmentReceiptNumbers.length > receiptIdx
      ? reservedInstallmentReceiptNumbers[receiptIdx++]
      : undefined;
    
    // Get student info for required fields
    const student = studentById.get(resolvedStudentId);
    ...
    return {
      ...
      studentId: resolvedStudentId,
      feeId: resolvedFeeId,
      studentName: (inst as any).studentName || student?.name || '',
      grade: (inst as any).grade || student?.grade || '',
      receiptNumber,
  ...
  for (const inst of installmentsWithIds) {
    const key = `${(inst as any).studentId}:${(inst as any).feeType}`;
    const paid = (inst as any).paidAmount !== undefined
      ? Number((inst as any).paidAmount)
      : ((inst as any).paidDate ? Number((inst as any).amount || 0) : 0);
    if ((inst as any).paidDate || (inst as any).isPaid) {
      paidByFee.set(key, (paidByFee.get(key) || 0) + paid);
    }
  }
  for (const f of feesWithIds) {
    const key = `${(f as any).studentId}:${(f as any).feeType}`;
    const paid = paidByFee.get(key);
    if (paid !== undefined) {
      (f as any).paid = Math.round(paid);
      const amount = Number((f as any).amount || 0);
      const discount = Number((f as any).discount || 0);
      (f as any).balance = Math.max(0, amount - discount - (f as any).paid);
      (f as any).status = (f as any).paid >= (amount - discount) ? 'paid' : ((f as any).paid > 0 ? 'partial' : 'unpaid');
    }
  }
...
export const saveImportedDataWithSync = async (
  ...
  try {
    ...
    const existingStudents = existingStudentsResponse.success ? existingStudentsResponse.data : [];
    const existingStudentIds = new Set(existingStudents.map((s: any) => s.studentId || s.student_id));
    const studentById = new Map<string, any>(existingStudents.map((s: any) => [s.id, s]));
    const allFeesResponse = await hybridApi.getFees(schoolId);
    const allFees = allFeesResponse.success ? allFeesResponse.data : [];
    const feeMap = new Map<string, any>();
    for (const f of allFees) {
      const sid = (f as any).studentId || (f as any).student_id;
      const ft = (f as any).feeType || (f as any).fee_type;
      if (sid && ft) feeMap.set(`${sid}:${ft}`, f);
    }
    const paidInstallmentsCountSeq = installments.filter((inst) => (inst as any).isPaid).length;
    let reservedNumbersSeq: string[] = [];
    if (paidInstallmentsCountSeq > 0) {
      try {
        reservedNumbersSeq = await reserveReceiptNumbers(schoolId, 'installment', paidInstallmentsCountSeq);
      } catch {}
    }
    let reservedIdxSeq = 0;
    ...
    for (const student of students) {
      ...
      try {
        ...
        // Add to existing IDs to prevent duplicates within import
        existingStudentIds.add(student.studentId);
        existingStudents.push(savedStudent);
        studentById.set(savedStudent.id, savedStudent);
    ...
    for (const fee of fees) {
      ...
      const student = studentById.get(realStudentId);
      if (!student) {
        console.log(`Student not found with ID: ${realStudentId}`);
        continue;
      }
      ...
      try {
        ...
        if (createFeeResponse.success) {
          ...
          savedFeesCount++;
          console.log(`Saved fee: ${fee.feeType} for student ${student.name} (${amount})`);
          feeMap.set(`${realStudentId}:${fee.feeType}`, savedFee);
        } else {
    ...
    for (const installment of installments) {
      ...
      try {
        ...
        if (!feeId) {
          const matchingFee = feeMap.get(`${realStudentId}:${installment.feeType}`);
          ...
          if (matchingFee) {
            ...
            if (createFeeResponse.success) {
              ...
              feeIdMap.set(`${installment.studentId}-${installment.feeType}`, feeId);
              savedFeesCount++;
              feeMap.set(`${realStudentId}:${installment.feeType}`, fee);
            } else {
        ...
        const importReceiptNumber = installment.isPaid && reservedNumbersSeq.length > reservedIdxSeq
          ? reservedNumbersSeq[reservedIdxSeq++]
          : undefined;
    ...
    const allFeesResponse2 = await hybridApi.getFees(schoolId);
    const allInstResponse = await hybridApi.getInstallments(schoolId);
    if (allFeesResponse2.success) {
      const savedFeeIds = Array.from(feeIdMap.values());
      const feesToUpdate = allFeesResponse2.data.filter(fee => 
