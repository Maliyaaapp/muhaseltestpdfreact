...
export const generateReceiptHTML = async (data: ReceiptData): Promise<string> => {
  ...
  // Skip verbose logging for bulk operations to improve performance
  const skipVerboseLogging = (data as any).skipApiCalls === true;
  ...
  try {
    ...
    // SKIP API CALLS if skipApiCalls flag is set (for bulk generation performance)
    else if (((data as any).studentInternalId || (data as any).studentId) && transportationFees === 0 && !skipVerboseLogging) {
      try {
        // Attempt to directly load the student's transportation fee
        const studentId = (data as any).studentInternalId || (data as any).studentId;
  ...
  // SKIP API CALLS if skipApiCalls flag is set (for bulk generation performance)
  else if ((data as any).studentId && !skipVerboseLogging) {
    try {
      // Try hybridApi first
  ...
  const explicitStatus: 'paid' | 'partial' | 'unpaid' = ((): any => {
    if (data.status === 'unpaid') return 'unpaid';
    if (data.status === 'partial') return 'partial';
    if (data.status === 'paid') return 'paid';
    const amt = Number(data.amount || 0);
    const rem = Number(data.remainingAmount || 0);
    if (amt === 0 && rem > 0) return 'unpaid';
    if (data.isPartialPayment) return 'partial';
    return 'paid';
  })();
  ...
  const isCombinedReceipt = (() => {
    const ft = (data as any).feeType || '';
    const ftLower = String(ft).toLowerCase();
    if (ftLower === 'transportation_and_tuition') return true;
    if (ft === 'رسوم مدمجة') return true;
    return false;
  })();
        ...
        <div class="receipt-body">
          <div style="text-align: center;" class="payment-status">
            <span class="status-badge ${explicitStatus === 'partial' ? 'status-partial' : (explicitStatus === 'unpaid' ? 'status-unpaid' : 'status-paid')}">
              ${isArabic 
                ? (explicitStatus === 'partial' ? 'دفعة جزئية' : (explicitStatus === 'unpaid' ? 'غير مدفوع' : 'مدفوع')) 
                : (explicitStatus === 'partial' ? 'Partial Payment' : (explicitStatus === 'unpaid' ? 'Unpaid' : 'Paid'))}
            </span>
          </div>
          ...
          <div class="info-section">
            <div class="info-group">
              ...
              <div class="info-row">
                ...
                <div dir="ltr">${(data as any).studentIdDisplay || data.studentId || ''}</div>
              </div>
              <div class="info-row">
            ...
            <div class="info-group">
              ...
              <div class="info-row">
                ...
                <div>
                  ${(() => {
                    const base = isInstallmentReceipt ? (isArabic ? 'رسوم القسط' : 'Installment Fees') : translateFeeType((data as any).feeType || '');
                    const isCombined = ((data as any).feeType || '').toLowerCase() === 'transportation_and_tuition' || base === 'رسوم مدمجة';
                    return isCombined ? `${base} (رسوم النقل و الرسوم الدراسية)` : base;
                  })()}
                </div>
              </div>
              <div class="info-row">
          ...
          <table class="fee-table" dir="rtl">
            ...
            <tbody>
              ...
              ${isInstallmentReceipt ? `
                ...
                <tr>
                  <td>${isArabic ? 'قيمة القسط' : 'Installment Total'}</td>
                  <td class="fee-amount">${Math.round(Number((data as any).totalAmount || 0)).toLocaleString('en-US')} ${currencySymbol}</td>
                </tr>
                <tr style="background-color: #f0f8ff; color: #2E8B57; font-weight: bold;">
                  <td><strong>${isArabic ? 'المبلغ المدفوع' : 'Amount Paid'}</strong></td>
                  <td class="fee-amount"><strong>${Math.round(Number((data as any).paidAmount || 0)).toLocaleString('en-US')} ${currencySymbol}</strong></td>
                </tr>
                <tr style="background-color: #fff5f5; color: #C53030; font-weight: bold;">
                  <td><strong>${isArabic ? 'المبلغ المتبقي' : 'Remaining Amount'}</strong></td>
                  <td class="fee-amount"><strong>${Math.round(Number((data as any).remainingAmount || 0)).toLocaleString('en-US')} ${currencySymbol}</strong></td>
                </tr>
              ` : `
                ${isCombinedReceipt ? `
