...
export const createSchool = async (schoolData: SchoolWithLogo): Promise<School> => {
  try {
    ...
    if (shouldUseSupabase()) {
      ...
      // Also save to localStorage for offline access (localStorage only, not Supabase)
      ...
      // Save directly to localStorage without going through Supabase again
      const schools = JSON.parse(localStorage.getItem('schools') || '[]');
      schools.push(localSchool);
